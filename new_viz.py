#!/usr/bin/env python
# new_viz.py - A completely rewritten visualization file for Schelling model
import numpy as np
...
class SchellingVisualization:
    """Visualization for the Schelling Segregation Model"""
    ...
    def __init__(self, width=20, height=20, density=1.0, homophily=0.3, num_agent_types=2):
        ...
        self.ani = animation.FuncAnimation(
            self.fig, self.animate, interval=100, save_count=50, blit=False
        )
    ...
    def setup_figure(self):
        ...
        gs = self.fig.add_gridspec(2, 2, width_ratios=[3, 1], height_ratios=[3, 1])
    ...
    def setup_controls(self):
        ...
        # Step button
        step_button_pos = [button_left, 0.7, button_width, button_height]
        self.ax_step = self.fig.add_axes(step_button_pos)
        self.step_button = Button(self.ax_step, 'Step')
        self.step_button.on_clicked(self.step_model)
        
        # Run/Stop button
        run_button_pos = [button_left, 0.55, button_width, button_height]
        self.ax_run = self.fig.add_axes(run_button_pos)
        self.run_button = Button(self.ax_run, 'Run')
        self.run_button.on_clicked(self.toggle_running)
        
        # Reset button
        reset_button_pos = [button_left, 0.4, button_width, button_height]
        self.ax_reset = self.fig.add_axes(reset_button_pos)
        self.reset_button = Button(self.ax_reset, 'Reset')
        self.reset_button.on_clicked(self.reset_model)
        
        # Save button
        save_button_pos = [button_left, 0.25, button_width/2-0.05, button_height]
        self.ax_save = self.fig.add_axes(save_button_pos)
        self.save_button = Button(self.ax_save, 'Save')
        self.save_button.on_clicked(self.save_model)
        
        # Load button
        load_button_pos = [button_left + button_width/2 + 0.05, 0.25, button_width/2-0.05, button_height]
        self.ax_load = self.fig.add_axes(load_button_pos)
        self.load_button = Button(self.ax_load, 'Load')
        self.load_button.on_clicked(self.load_model)
        
        # Pattern selection
        pattern_radios_pos = [button_left, 0.0, button_width, 0.15]
        self.ax_pattern = self.fig.add_axes(pattern_radios_pos)
        ...
        # View mode selection
        view_radios_pos = [0.0, 0.6, 0.15, button_width]
        self.ax_view = self.fig.add_axes(view_radios_pos)
        ...
        # Homophily slider
        homophily_slider_pos = [0.25, 0.1, button_width, 0.03]
        self.ax_homophily = self.fig.add_axes(homophily_slider_pos)
        ...
        # Agent types slider
        agent_types_slider_pos = [0.25, 0.05, button_width, 0.03]
        self.ax_agent_types = self.fig.add_axes(agent_types_slider_pos)
    ...
    def update_grid(self):
        ...
        if self.view_mode == 'normal':
            ...
            if not hasattr(self, 'cbar') or self.view_mode != getattr(self, 'last_view_mode', None):
    ...
    def update_metrics_plot(self):
        ...
        max_x = max(len(self.x_data) + 1, 10)
    ...
    def save_model(self, event=None):
        ...
        try:
            ...
            filename = f"saved_models/schelling_model_{timestamp}.pickle"
            ...
            # Update status
            self.status_text.set_text(f"Model saved to:\n{filename}")
            self.fig.canvas.draw_idle()
            
        except Exception as e:
            self.status_text.set_text(f"Error saving model:\n{str(e)}")
            self.fig.canvas.draw_idle()
    ...
    def load_model(self, event=None):
        ...
        try:
            ...
            if not os.path.exists('saved_models'):
                self.status_text.set_text("No saved models found")
                self.fig.canvas.draw_idle()
                return
            
            # Get a list of saved models
            saved_files = [f for f in os.listdir('saved_models') if f.endswith('.pickle')]
            if not saved_files:
                self.status_text.set_text("No saved models found")
                self.fig.canvas.draw_idle()
                return
            ...
            saved_files.sort(key=lambda f: os.path.getmtime(os.path.join('saved_models', f)), reverse=True)
            
            # Load the newest file
            filename = os.path.join('saved_models', saved_files[0])
            ...
            self.view_mode = data['view_mode']
            self.steps = data['steps']
            self.x_data = data['x_data']
            self.happiness_data = data['happiness_data']
            self.segregation_data = data['segregation_data']
            
            # Update the widgets to match loaded state
            self.homophily_slider.set_val(self.model.homophily)
            self.agent_types_slider.set_val(self.model.num_agent_types)
            
            # Update the view mode radio button
            view_index = ['normal', 'network', 'happiness'].index(self.view_mode)
            self.view_radio.set_active(view_index)
            
            # Update pattern radio button
            pattern_index = ['random', 'alternating', 'clusters', 'stripes'].index(self.current_pattern)
            self.pattern_radio.set_active(pattern_index)
            
            # Update the visualization
            self.update_grid()
            
            # Update status
            self.status_text.set_text(f"Model loaded from:\n{filename}")
            self.fig.canvas.draw_idle()
            
        except Exception as e:
            self.status_text.set_text(f"Error loading model:\n{str(e)}")
            self.fig.canvas.draw_idle()
...
if __name__ == "__main__":
    print("Starting Schelling Segregation Model with Improved Visualization...")
